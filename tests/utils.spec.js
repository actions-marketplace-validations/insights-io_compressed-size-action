...
test("diffTable", () => {
  ...
  expect(
    diffTable(files, { ...defaultOptions, showTotal: false })
  ).toMatchSnapshot();
  expect(
    diffTable(files, { ...defaultOptions, collapseUnchanged: false })
  ).toMatchSnapshot();
  expect(
    diffTable(files, { ...defaultOptions, omitUnchanged: true })
  ).toMatchSnapshot();
  expect(
    diffTable(files, { ...defaultOptions, minimumChangeThreshold: 10 })
  ).toMatchSnapshot();
  expect(
    diffTable(files, { ...defaultOptions, title: "Test title" })
  ).toMatchSnapshot();
...
test("stripHash", () => {
  ...
  expect(stripHash("\\.(\\w{5})\\.chunk\\.js$")("foo.abcde.chunk.js")).toBe(
    "foo.*****.chunk.js"
  );
